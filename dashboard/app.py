...
def run_inference(caption):
    fake_keywords = [
        ...
        "truth", "hiding", "agenda", "suppressed", "leaked", "control",
        "track", "doctors", "share", "before", "they"
    ]
    ...
    top_words   = [w.strip(".,!?") for w in caption.split()
                   if w.strip(".,!?").lower() in fake_keywords][:4]
...
with col2:
    ...
    if analyze_btn and caption.strip():
        with st.spinner("Running VerifAI pipeline..."):
            fake_prob, cluster_id, top_words = run_inference(caption)
        ...
        st.code(
            "SHOCKING: Government secretly putting microchips in vaccines "
            "to track and control citizens. Doctors are hiding the truth!"
        )
        st.code(
            "City council approves new budget for road maintenance and "
            "public transport improvements across the district."
        )
...
if analyze_btn and caption.strip():
        ...
        if show_shap:
            if top_words:
                for word in top_words:
                    st.markdown(
                        f'<span style="background:#ffe0e0;padding:4px 12px;'
                        f'border-radius:4px;margin:3px;display:inline-block;'
                        f'font-weight:500;">⚠️ {word}</span>',
                        unsafe_allow_html=True
                    )
            else:
    ...
    if is_fake:
        st.error(
            ...
            f"as potential misinformation. Assigned to narrative cluster "
            f"**#{cluster_id}**, which groups similar misleading posts together."
        )
    else:
        st.success(
            f"VerifAI found no strong misinformation signals in this post "
            f"({fake_prob:.0%} risk score). "
            f"Assigned to narrative cluster **#{cluster_id}**."
        )